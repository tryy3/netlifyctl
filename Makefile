...
build: test
	...
	GOOS=$(os) GOARCH=$(arch) go build -ldflags "-X github.com/tryy3/netlifyctl/commands.Version=${TAG} -X github.com/tryy3/netlifyctl/commands.SHA=`git rev-parse HEAD`"
...
release: ## Upload release to GitHub releases.
	...
	GOOS=darwin GOARCH=$(arch) go build -ldflags "-X github.com/tryy3/netlifyctl/commands.Version=${TAG} -X github.com/tryy3/netlifyctl/commands.SHA=`git rev-parse HEAD`" -o builds/darwin-${TAG}/netlifyctl
	mkdir -p builds/linux-${TAG}
	GOOS=linux GOARCH=$(arch) go build -ldflags "-X github.com/tryy3/netlifyctl/commands.Version=${TAG} -X github.com/tryy3/netlifyctl/commands.SHA=`git rev-parse HEAD`" -o builds/linux-${TAG}/netlifyctl
	mkdir -p builds/windows-${TAG}
	GOOS=windows GOARCH=$(arch) go build -ldflags "-X github.com/tryy3/netlifyctl/commands.Version=${TAG} -X github.com/tryy3/netlifyctl/commands.SHA=`git rev-parse HEAD`" -o builds/windows-${TAG}/netlifyctl.exe
	@rm -rf releases/${TAG}
